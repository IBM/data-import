...
def extractObservable(args, obs, values):
    ...
    if val:
        if (not isinstance(val, basestring)) and isinstance(val,
...
def get_parser():
    ...
    parser.add_option('-r', '--referenceset',
                      ...
                      action='store')
    parser.add_option('-i', '--ip', default="127.0.0.1",
                      help='IP or Host of the QRadar console',
                      action='store')
    parser.add_option('-t', '--token', help='QRadar authorized service token',
                      action='store')
    ...
    parser.add_option('-x', '--taxii',
                      ...
                      action='store')
    parser.add_option('-p', '--taxiiport', default="80",
...
def main():
    ...
    # Import from a TAXII server
        ...
        resp = client.callTaxiiService2(args[0].taxii,
                                        args[0].taxii_endpoint + "/poll/",
